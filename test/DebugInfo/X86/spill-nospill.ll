...
; DWARF: Location description: 77 24
; DWARF: Location description: 50
; DWARF: Location description: 54

; TODO: Re-enable the check-lines below and disable the DWARF ones above.
; The check-lines above tests the expressions 'DW_OP_breg7 RSP+36',
; 'DW_OP_reg0 RAX', and 'DW_OP_reg4 RSI' respectively,
; similar to the checks below, but then in a non-pretty-printed form.

; *****:      DW_TAG_variable
; *****-NEXT:   DW_AT_location        (
; *****-NEXT:      {{.*}} - {{.*}}: DW_OP_breg7 RSP+36
; *****-NEXT:      {{.*}} - {{.*}}: DW_OP_reg0 RAX)
; *****-NEXT:   DW_AT_name    ("x")
