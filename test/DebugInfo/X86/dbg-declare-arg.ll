...
; CHECK: Location description: 77 08 06

; TODO: Re-enable the check-lines below and disable the one above.
; The check-line above tests the expression 'DW_OP_breg7 RSP+8, DW_OP_deref'
; similar to the check below, but then in a non-pretty-printed form.

; *****: DW_AT_name {{.*}}"j"
; *****: DW_TAG_variable
; *****-NEXT:   DW_AT_location [DW_FORM_sec_offset] (
; *****-NEXT:     0x{{.*}} - 0x{{.*}}: DW_OP_breg7 RSP+8, DW_OP_deref)
; *****-NEXT:   DW_AT_name {{.*}}"my_a"
