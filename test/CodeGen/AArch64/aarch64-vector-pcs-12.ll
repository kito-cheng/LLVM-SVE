; RUN: llc -mtriple=aarch64-linux-gnu -aarch64-vector-pcs=12 < %s | FileCheck %s

; Check that correct registers are preserved by the callee
; when using aarch64 vector pcs. We should expect registers
; Q12 and Q23 to be preserved, but Q5 is not.

define aarch64_vector_pcs void @callee(){
; CHECK-LABEL: callee:
; CHECK-NOT: q5
; CHECK: stp q23, q12
call void asm sideeffect "nop", "~{q5},~{q12},~{q23}"() nounwind
ret void
}
