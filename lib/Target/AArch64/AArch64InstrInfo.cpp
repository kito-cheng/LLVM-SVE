...
void llvm::emitFrameOffset(MachineBasicBlock &MBB,
                           ...
                           bool KillSrcReg) {
  // If we write to the same register as SrcReg, we always kill it
  if (DestReg == SrcReg)
    KillSrcReg = true;
  ...
  while (((unsigned)Offset) >= (1 << ShiftSize)) {
    ...
    BuildMI(MBB, MBBI, DL, TII->get(Opc), DestReg)
        .addReg(SrcReg, KillSrcReg ? RegState::Kill : 0)
...
MachineInstr *AArch64InstrInfo::foldMemoryOperandImpl(
  ...
  if (MI.isCopy() && Ops.size() == 1 &&
    ...
    bool isSVERegion = MFI.getObjectRegion(FrameIndex) != 0;
    ...
    if (IsSpill && DstMO.isUndef() &&
      ...
      case AArch64::ssub:
        if (AArch64::GPR32RegClass.contains(SrcReg)) {
          ...
          SpillRC = isSVERegion ? &AArch64::ZPRRegClass
...
int llvm::isAArch64FrameOffsetLegal(const MachineInstr &MI, int &Offset,
  ...
  if (Offset >= MinOff && Offset <= MaxOff) {
    ...
    if (IsMulVL && ((Scale % 16) != 0)) {
      // Restrict NewOff so the remaining Offset is 16-byte aligned.
      assert(Scale == 2 && "Expected predicate scaled offset!");
      NewOff = (NewOff / 8) * 8;
    }
...
bool llvm::rewriteAArch64FrameIndex(MachineInstr &MI, unsigned FrameRegIdx,
  ...
  auto &TFI = MI.getParent()->getParent()->getFrameInfo();
  if (Opcode == AArch64::ADDSXri || Opcode == AArch64::ADDXri) {
    Offset += MI.getOperand(ImmIdx).getImm();
    // Both AArch64 and SVE (ab)use the ADDXri instruction
    // to materialise a FrameIndex.
    if (TFI.getObjectRegion(MI.getOperand(FrameRegIdx).getIndex())) {
      auto &MBB = *MI.getParent();
      auto &MF = *MBB.getParent();
      SVEVecStackRegion::adjustRegBySVE(MF, MBB, MI, MI.getDebugLoc(), FrameReg,
                                        std::abs(Offset), Offset >= 0,
                                        /*KillBaseReg=*/IsTemporaryFrameReg,
...
static int calcOperandDist(const MachineInstr &Root, int OpdIdx) {
  ...
  return std::distance(OpdInstr->getIterator(), Root.getIterator());
}
